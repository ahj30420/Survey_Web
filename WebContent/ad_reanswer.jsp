...
<body>
    ...
    <main>
    	<%
    		...
    		session.setAttribute("ansiqind",inqdto.get(i).getIqind());
    	%>
        <div class="title">답변 달기</div>
        <div class="question">
            <div class="qu">
                <div class="d"><div>Q</div><div><%=inqdto.get(i).getConinquiry() %></div><div><%=inqdto.get(i).getRedate() %></div></div>
                <form action="ad_reanswer.do">
                    <div class="qu_con">
                        <div>A</div>
                        <div><textarea rows="8" cols="83" name="ansinquiry" placeholder="답변을 작성해주세요."></textarea></div>
